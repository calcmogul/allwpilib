...
class CANTalon : public MotorSafety,
 ...
 public:
   ...
   * to ensure changes in output take effect before performing buffering
   * actions.
   ...
   * is
   *   effectively disconnected from the executer, allowing the robot to
   * gracefully
   ...
   *   If the active traj is empty, and so is the buffer, the motor drive is
   * neutral and
   *   isUnderrun is set.  When active traj times out, and buffer has at least
   * one point,
   *   Talon shifts in next one, and isUnderrun is cleared.  When active traj
   * times out,
   *   and buffer is empty, Talon keeps processing active traj and sets
   * IsUnderrun.
   ...
   * API (top).
   * Be sure to check GetMotionProfileStatus() to know when the buffer is
   * actually cleared.
   */
   ...
   * API's top level buffer to
   * the Talon's bottom level buffer.  Recommendation is to call this twice as
   * fast as the executation rate of the motion profile.
   * So if MP is running with 20ms trajectory points, try calling this routine
   * every 10ms.  All motion profile functions are thread-safe
   ...
   * until
   * Robot Application clears it with this routine, which ensures Robot
   * Application
   * gets a chance to instrument or react.  Caller could also check the
   * isUnderrun flag
 ...
 private:
   ...
   * number of turns a full voltage sweep represents is necessary for
   * calculating rotations
   * and velocity.
   * A default value of zero means the API behaves as it did during the 2015
   * season, there are 1024
   ...
   * selection is helpful at the API level
   ...
   * @return        The number of native Talon units per rotation of the
   * selected sensor.
   ...
   * rotations of selected feedback sensor.
   *                            If user has never called the config routine for
   * the selected sensor, then the caller
   *                            is likely passing rotations in engineering units
   * already, in which case it is returned
   *                            as is.
   *                            @see ConfigPotentiometerTurns
   *                            @see ConfigEncoderCodesPerRev
   ...
   * minute of selected feedback sensor.
   *                            If user has never called the config routine for
   * the selected sensor, then the caller
   *                            is likely passing rotations in engineering units
   * already, in which case it is returned
   ...
   * Talon SRX units.
   *                            If user has never called the config routine for
   * the selected sensor, then the return
   *                            will be in TALON SRX units as well to match the
   * behavior in the 2015 season.
   *                            @see ConfigPotentiometerTurns
   *                            @see ConfigEncoderCodesPerRev
