...
bool ConvertUTF8toWide(unsigned WideCharWidth, std::string_view Source,
  ...
  if (WideCharWidth == 1) {
    ...
    if (!isLegalUTF8String(&Pos, reinterpret_cast<const UTF8*>(
                                     Source.data() + Source.size()))) {
      result = sourceIllegal;
      ErrorPtr = Pos;
    } else {
      std::memcpy(ResultPtr, Source.data(), Source.size());
...
bool convertUTF16ToUTF8String(std::span<const char> SrcBytes,
  ...
  const UTF16* SrcEnd =
      reinterpret_cast<const UTF16*>(&*SrcBytes.begin() + SrcBytes.size());
...
bool convertUTF16ToUTF8String(std::span<const UTF16> Src,
                              SmallVector<char>& Out) {
...
bool convertUTF32ToUTF8String(std::span<const char> SrcBytes,
                              std::string& Out) {
  assert(Out.empty());
  ...
  const UTF32* SrcEnd =
      reinterpret_cast<const UTF32*>(&*SrcBytes.begin() + SrcBytes.size());
...
bool convertUTF8ToUTF16String(std::string_view SrcUTF8,
  ...
  const UTF8* SrcEnd =
      reinterpret_cast<const UTF8*>(SrcUTF8.data() + SrcUTF8.size());
...
bool convertWideToUTF8(const std::wstring& Source, SmallVector<char>& Result) {
  if (sizeof(wchar_t) == 1) {
    ...
    std::memcpy(&Result[0], Source.data(), Source.size());
    return true;
