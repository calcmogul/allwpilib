...
#ifndef WPIUTIL_WPI_POINTERUNION_H_
#define WPIUTIL_WPI_POINTERUNION_H_
...
struct TypesAreDistinct
    : std::integral_constant<bool,
                             !std::disjunction_v<std::is_same<T, Us>...> &&
                                 TypesAreDistinct<Us...>::value> {};
template <typename T>
struct TypesAreDistinct<T> : std::true_type {};
}  // namespace detail
...
struct TypesAreDistinct;
template <>
struct TypesAreDistinct<> : std::true_type {};
template <typename... Ts>
struct TypesAreDistinct
...
class PointerUnionMembers<Derived, ValTy, I> {
 protected:
  ...
  PointerUnionMembers(ValTy Val) : Val(Val) {}  // NOLINT
...
class PointerUnionMembers<Derived, ValTy, I, Type, Types...>
 ...
 public:
  using Base::Base;
  PointerUnionMembers() = default;
  PointerUnionMembers(Type V)  // NOLINT
      : Base(ValTy(
  ...
  Derived& operator=(Type V) {
    this->Val = ValTy(
        const_cast<void*>(PointerLikeTypeTraits<Type>::getAsVoidPointer(V)), I);
...
///    P = (int*)0;
///    std::printf("%d %d", P.is<int*>(), P.is<float*>());  // prints "1 0"
///    X = P.get<int*>();     // ok.
...
#endif  // WPIUTIL_WPI_POINTERUNION_H_
