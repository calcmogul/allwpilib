...
class SmallVectorMemoryBuffer : public MemoryBuffer {
 public:
  ...
  /// r-value.
  SmallVectorMemoryBuffer(SmallVector<uint8_t>&& sv)  // NOLINT
      : m_sv(std::move(sv)), m_bufferName("<in-memory object>") {
    Init(this->m_sv.data(), this->m_sv.data() + this->m_sv.size());
  }

  /// Construct a named SmallVectorMemoryBuffer from the given
  /// SmallVector r-value and StringRef.
  SmallVectorMemoryBuffer(SmallVector<uint8_t>&& sv, std::string_view name)
