...
using __remove_cvref_t = remove_cv_t<remove_reference_t<_T>>;

template <typename _T1, typename _T2>
struct __ref_pair {
  static_assert(is_reference<_T1>{} && is_reference<_T2>{});

  using __pair_type = pair<__remove_cvref_t<_T1>, __remove_cvref_t<_T2>>;
  using __pair_of_references_type =
      pair<__remove_cvref_t<_T1>&, __remove_cvref_t<_T2>&>;
  using __const_pair_of_references_type =
      pair<__remove_cvref_t<_T1> const&, __remove_cvref_t<_T2> const&>;
...
struct __flat_map_iterator {
  ...
  using value_type = pair<__remove_cvref_t<_KeyRef>, __remove_cvref_t<_TRef>>;
  using difference_type = typename iterator_traits<_KeyIter>::difference_type;
  ...
  __flat_map_iterator(
      ...
      enable_if_t<is_convertible<_TRef2, _TRef>::value &&
...
inline void swap(pair<_KeyRef, _TRef>&& __lhs, pair<_KeyRef, _TRef>&& __rhs) {
  using std::swap;
  swap(__lhs.first, __rhs.first);
  swap(__lhs.second, __rhs.second);
}
...
class flat_map {
  ...
  using __uses = enable_if_t<uses_allocator<_KeyContainer, _Alloc>::value &&
  ...
  struct __has_begin_end<_Container,
                         void_t<decltype(std::begin(declval<_Container>())),
                                decltype(std::end(declval<_Container>()))>>
      : true_type {};
  template <typename _Container>
 ...
 public:
  ...
  template <class _Alloc, class _Enable = __uses<_Alloc>>
  flat_map(initializer_list<value_type>&& __il, const key_compare& __comp,
           const _Alloc& __a)
  ...
  template <class... _Args,
            class _Enable = enable_if_t<is_constructible<
                pair<key_type, mapped_type>, _Args&&...>::value>>
  ...
  template <class _P, class _Enable = enable_if_t<is_constructible<
                          pair<key_type, mapped_type>, _P&&>::value>>
...
#if USE_CONCEPTS
    ranges::inplace_merge(__mutable_first, __inserted_first, __inserted_last,
                          value_comp());
#else
    inplace_merge(__mutable_first, __inserted_first, __inserted_last,
                  value_comp());
#endif
  }
  template <class _InputIterator>
  ...
  template <class _M, class _Enable = enable_if_t<
...
#else
    return std::lower_bound(__c.keys.begin(), __c.keys.end(), __k, __compare);
#endif
  }
  template <typename _K>
...
#else
    return std::upper_bound(__c.keys.begin(), __c.keys.end(), __k, __compare);
#endif
  }
  template <typename _K>
