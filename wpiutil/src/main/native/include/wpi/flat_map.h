...
//
// https://github.com/tzlaine/flat_map

#ifndef WPIUTIL_WPI_FLAT_MAP_H_
#define WPIUTIL_WPI_FLAT_MAP_H_
...
struct __ref_pair {
  ...
  operator __pair_type() const { return __pair_type(first, second); }  // NOLINT
  operator __pair_of_references_type() const {                         // NOLINT
    return __pair_of_references_type(first, second);
  }
  operator __const_pair_of_references_type() const {  // NOLINT
...
template <typename _T1, typename _T2>
void swap(__ref_pair<_T1, _T2> const& __lhs,
          __ref_pair<_T1, _T2> const& __rhs) {
  using std::swap;
...
struct __flat_map_iterator {
  ...
  __flat_map_iterator(  // NOLINT
      __flat_map_iterator<_KeyRef, _TRef2, _KeyIter, _MappedIter2> __other,
      std::enable_if_t<std::is_convertible_v<_TRef2, _TRef> &&
  ...
  friend bool operator==(__flat_map_iterator __lhs, __flat_map_iterator __rhs) {
  ...
  friend typename __flat_map_iterator::difference_type operator-(
      __flat_map_iterator __lhs, __flat_map_iterator __rhs) {
    return __lhs.__key_it_ - __rhs.__key_it_;
  }
...
inline void swap(std::pair<_KeyRef, _TRef>&& __lhs,
                 std::pair<_KeyRef, _TRef>&& __rhs) {
  using std::swap;
  swap(__lhs.first, __rhs.first);
  swap(__lhs.second, __rhs.second);
}
...
class flat_map {
  ...
  using __uses =
      std::enable_if_t<std::uses_allocator<_KeyContainer, _Alloc>::value &&
                       std::uses_allocator<_MappedContainer, _Alloc>::value>;

  template <typename _Container, typename = void>
  struct __has_begin_end : std::false_type {};
  template <typename _Container>
  struct __has_begin_end<
      _Container, std::void_t<decltype(std::begin(std::declval<_Container>())),
                              decltype(std::end(std::declval<_Container>()))>>
      : std::true_type {};
  template <typename _Container>
  using __container = std::enable_if_t<__has_begin_end<_Container>::value>;
 ...
 public:
  ...
  using iterator =
      __flat_map_iterator<const key_type&, mapped_type&,
  ...
  class value_compare {
   ...
   private:
    key_compare __comp;
    value_compare(key_compare __c) : __comp(__c) {}  // NOLINT
  ...
  flat_map(key_container_type __key_cont, mapped_container_type __mapped_cont)
    ...
    std::ranges::sort(__first, __last, value_comp());
  }
  template <class _Alloc, class _Enable = __uses<_Alloc>>
  flat_map(const key_container_type& __key_cont,
           const mapped_container_type& __mapped_cont, const _Alloc& __a)
  ...
  flat_map(sorted_unique_t, key_container_type __key_cont,
      ...
      : __c{std::move(__key_cont), std::move(__mapped_cont)},
        __compare(key_compare()) {}
  template <class _Alloc, class _Enable = __uses<_Alloc>>
  flat_map(sorted_unique_t, const key_container_type& __key_cont,
           const mapped_container_type& __mapped_cont, const _Alloc& __a)
      : __c{key_container_type(__key_cont, __a),
            mapped_container_type(__mapped_cont, __a)},
        __compare() {}
  ...
  template <class _Container, class _Alloc,
            class _Enable1 = __container<_Container>,
            class _Enable2 = __uses<_Alloc>>
  flat_map(sorted_unique_t __s, const _Container& __cont, const _Alloc& __a)
      : __c{key_container_type(__a), mapped_container_type(__a)}, __compare() {
  ...
  template <class _InputIterator, class _Alloc, class _Enable = __uses<_Alloc>>
  flat_map(_InputIterator __first, _InputIterator __last,
  ...
  template <class _InputIterator>
  flat_map(sorted_unique_t __s, _InputIterator __first, _InputIterator __last,
           const key_compare& __comp = key_compare())
      : __c(), __compare(__comp) {
  ...
  flat_map(std::initializer_list<value_type>&& __il,
           const key_compare& __comp = key_compare())
      : flat_map(__il, __comp) {}
  template <class _Alloc, class _Enable = __uses<_Alloc>>
  flat_map(std::initializer_list<value_type>&& __il, const key_compare& __comp,
           const _Alloc& __a)
      : flat_map(std::begin(__il), std::end(__il), __comp, __a) {}
  template <class _Alloc, class _Enable = __uses<_Alloc>>
  flat_map(std::initializer_list<value_type>&& __il, const _Alloc& __a)
      : flat_map(std::begin(__il), std::end(__il), key_compare(), __a) {}
  flat_map(sorted_unique_t __s, std::initializer_list<value_type>&& __il,
           const key_compare& __comp = key_compare())
  ...
  flat_map& operator=(std::initializer_list<value_type> __il) {
  ...
  iterator end() noexcept { return iterator(__c.keys.end(), __c.values.end()); }
  ...
  const_reverse_iterator rbegin() const noexcept {
    return const_reverse_iterator(__c.keys.rbegin(), __c.values.rbegin());
  }
  ...
  bool empty() const noexcept {
    return __c.keys.empty();
  }
  size_type size() const noexcept { return __c.keys.size(); }
  size_type max_size() const noexcept {
    return std::min<size_type>(__c.keys.max_size(), __c.values.max_size());
  }
  ...
  template <class... _Args,
            class _Enable = std::enable_if_t<std::is_constructible_v<
                std::pair<key_type, mapped_type>, _Args&&...>>>
  std::pair<iterator, bool> emplace(_Args&&... __args) {
    std::pair<key_type, mapped_type> __p(std::forward<_Args>(__args)...);
    return try_emplace(std::move(__p.first), std::move(__p.second));
  }
  template <class... _Args,
            class _Enable = std::enable_if_t<std::is_constructible_v<
                std::pair<key_type, mapped_type>, _Args&&...>>>
  iterator emplace_hint(const_iterator __position, _Args&&... __args) {
  ...
  template <class _P, class _Enable = std::enable_if_t<std::is_constructible_v<
                          std::pair<key_type, mapped_type>, _P&&>>>
  std::pair<iterator, bool> insert(_P&& __x) {
  ...
  void insert(_InputIterator __first, _InputIterator __last) {
    ...
    __mutable_iterator __inserted_first(__c.keys.begin() + __prev_size,
                                        __c.values.begin() + __prev_size);
    __mutable_iterator __inserted_last(__c.keys.end(), __c.values.end());
    std::ranges::sort(__inserted_first, __inserted_last, value_comp());

    if (!__prev_size) {
      return;
    }

    __mutable_iterator __mutable_first(__c.keys.begin(), __c.values.begin());
    std::ranges::inplace_merge(__mutable_first, __inserted_first,
                               __inserted_last, value_comp());
  }
  template <class _InputIterator>
  ...
  void insert(std::initializer_list<value_type> __il) {
    insert(__il.begin(), __il.end());
  }
  void insert(sorted_unique_t __s, std::initializer_list<value_type> __il) {
  ...
  void replace(key_container_type&& __key_cont,
               mapped_container_type&& __mapped_cont) {
    __scoped_clear _(this);
  ...
  template <class... _Args,
            class _Enable = std::enable_if_t<
                std::is_constructible_v<mapped_type, _Args&&...>>>
  std::pair<iterator, bool> try_emplace(const key_type& __k,
                                        _Args&&... __args) {
    auto __it = __key_lower_bound(__k);
    if (__it == __c.keys.end() || __compare(*__it, __k) ||
        __compare(__k, *__it)) {
      auto __values_it =
          __c.values.emplace(__project(__it), std::forward<_Args>(__args)...);
      __it = __c.keys.insert(__it, __k);
      return std::pair<iterator, bool>(iterator(__it, __values_it), true);
    }
    return std::pair<iterator, bool>(iterator(__it, __project(__it)), false);
  }
  template <class... _Args,
            class _Enable = std::enable_if_t<
                std::is_constructible_v<mapped_type, _Args&&...>>>
  ...
  iterator try_emplace(const_iterator __hint, key_type&& __k,
                       _Args&&... __args) {
    return try_emplace(std::forward<key_type>(__k),
                       std::forward<_Args>(__args)...)
        .first;
  }

  template <class _M, class _Enable = std::enable_if_t<
                          std::is_assignable_v<mapped_type&, _M> &&
                          std::is_constructible_v<mapped_type, _M&&>>>
  std::pair<iterator, bool> insert_or_assign(const key_type& __k, _M&& __obj) {
    auto __it = __key_lower_bound(__k);
    if (__it == __c.keys.end() || __compare(*__it, __k) ||
        __compare(__k, *__it)) {
      auto __values_it =
  ...
  template <class _M, class _Enable = std::enable_if_t<
                          std::is_assignable_v<mapped_type&, _M> &&
                          std::is_constructible_v<mapped_type, _M&&>>>
  std::pair<iterator, bool> insert_or_assign(key_type&& __k, _M&& __obj) {
    auto __it = __key_lower_bound(__k);
    if (__it == __c.keys.end() || __compare(*__it, __k) ||
        __compare(__k, *__it)) {
      auto __values_it =
  ...
  template <class _M, class _Enable = std::enable_if_t<
                          std::is_assignable_v<mapped_type&, _M> &&
                          std::is_constructible_v<mapped_type, _M&&>>>
  iterator insert_or_assign(const_iterator hint, const key_type& __k,
                            _M&& __obj) {
  ...
  template <class _M, class _Enable = std::enable_if_t<
                          std::is_assignable_v<mapped_type&, _M> &&
                          std::is_constructible_v<mapped_type, _M&&>>>
  iterator insert_or_assign(const_iterator hint, key_type&& __k, _M&& __obj) {
    return insert_or_assign(std::forward<key_type>(__k),
                            std::forward<_M>(__obj))
        .first;
  }

  iterator erase(iterator __position) {
    return iterator(__c.keys.erase(__position.__key_iter()),
  ...
  iterator erase(const_iterator __first, const_iterator __last) {
    return iterator(
        ...
        __c.values.erase(__first.__mapped_iter(), __last.__mapped_iter()));
  }

  void swap(flat_map& __fm) noexcept(std::is_nothrow_swappable_v<key_compare>) {
    using std::swap;
  ...
  const mapped_container_type& values() const noexcept { return __c.values; }
  ...
  std::pair<iterator, iterator> equal_range(const key_type& __k) {
    ...
    iterator const __last = find_if(__first, end(), [&](auto const& __x) {
  ...
  std::pair<const_iterator, const_iterator> equal_range(
    ...
    const_iterator const __last = find_if(__first, end(), [&](auto const& __x) {
      return __compare(__k, __x.first);
    });
    return pair<const_iterator, const_iterator>(__first, __last);
  }
  template <class _K>
  std::pair<iterator, iterator> equal_range(const _K& __k) {
    iterator const __first = lower_bound(__k);
    iterator const __last = find_if(__first, end(), [&](auto const& __x) {
      return __compare(__k, __x.first);
    });
  ...
  template <class _K>
  std::pair<const_iterator, const_iterator> equal_range(const _K& __k) const {
    const_iterator const __first = lower_bound(__k);
    const_iterator const __last = find_if(__first, end(), [&](auto const& __x) {
      return __compare(__k, __x.first);
    });
  ...
  friend bool operator==(const flat_map& __x, const flat_map& __y) {
    return std::ranges::equal(__x, __y);
  }
  friend bool operator!=(const flat_map& __x, const flat_map& __y) {
    return !(__x == __y);
  }
  ...
  friend void swap(flat_map& __x,
                   flat_map& __y) noexcept(noexcept(__x.swap(__y))) {
    return __x.swap(__y);
  }
 ...
 private:
  ...
  using __mutable_iterator = __flat_map_iterator<key_type&, mapped_type&,
                                                 __key_iter_t, __mapped_iter_t>;
  ...
  __key_iter_t __key_lower_bound(const _K& __k) {
    return std::ranges::lower_bound(__c.keys, __k, __compare);
  }
  template <typename _K>
  ...
  __key_iter_t __key_upper_bound(const _K& __k) {
    return std::ranges::upper_bound(__c.keys, __k, __compare);
  }
  template <typename _K>
