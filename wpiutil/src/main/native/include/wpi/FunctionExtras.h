...
#ifndef WPIUTIL_WPI_FUNCTIONEXTRAS_H_
#define WPIUTIL_WPI_FUNCTIONEXTRAS_H_
...
using EnableUnlessSameType = std::enable_if_t<
    !std::is_same<std::remove_cvref_t<CallableT>, ThisT>::value>;
...
class UniqueFunctionBase {
 protected:
  ...
  UniqueFunctionBase(UniqueFunctionBase&& RHS) noexcept {
    ...
    if (!isInlineStorage()) {
      ...
      std::memcpy(getInlineStorage(), RHS.getInlineStorage(),
                  InlineStorageSize);
    } else {
    ...
    std::memset(RHS.getInlineStorage(), 0xAD, InlineStorageSize);
#endif
  }
 ...
 public:
  explicit operator bool() const {
    return static_cast<bool>(CallbackAndInlineFlag.getPointer());
  }
};
...
#endif  // WPIUTIL_WPI_FUNCTIONEXTRAS_H_
