...
/**
 * TimesliceRobot extends the TimedRobot robot program framework to offer better real-time
 * performance.
 *
...
public class TimesliceRobot extends TimedRobot {
  public static final int kDefaultPriority = 15;
  public static final double kDefaultControllerPeriod = 0.005;

  /**
   * Constructor for TimesliceRobot.
   *
   * <p>The default robot periodic allocation is 2 ms, the default controller period is 5 ms, and
   * the default robot thread RT priority is 15. The priority can be changed later with
   * setPriority().
   */
  public TimesliceRobot() {
    this(0.002);
  }
   ...
   * Constructor for TimesliceRobot.
   *
   * <p>The default controller period is 5 ms and the default robot thread RT priority is 15. The
   * priority can be changed later with setPriority().
   *
   * @param robotPeriodicAllocation The allocation in seconds to give the TimesliceRobot periodic
   *     functions.
   */
  public TimesliceRobot(double robotPeriodicAllocation) {
    this(robotPeriodicAllocation, kDefaultControllerPeriod);
  }

  /**
   * Constructor for TimesliceRobot.
   *
   * <p>The default robot thread RT priority is 15. This can be changed later with setPriority().
   *
   * @param robotPeriodicAllocation The allocation in seconds to give the TimesliceRobot periodic
   *     functions.
  ...
  /**
   * Set priority of the main robot thread. The robot periodic functions and the controller periodic
   * functions will run with this priority.
   *
   * <p>The main robot thread's priority is 15 by default.
   *
   * @param priority Priority to set the main robot thread to (1-99 with 99 being highest). See "man
   *     7 sched" for more details.
   */
  public void setPriority(int priority) {
    if (!Threads.setCurrentThreadPriority(true, priority)) {
      throw new IllegalStateException(
          "Setting TimesliceRobot RT priority to " + priority + " failed\n");
    }
  }
  ...
  private double m_controllerPeriod;
