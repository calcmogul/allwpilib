...
void testFollowTrajectory(
  ...
  estimator.ResetPosition(xWheelPos, yWheelPos, wpi::math::Rotation3d{},
  ...
  wpi::math::Matrixd<2, 3> inverseKinematicsMatrix =
  ...
  while (t < trajectory.Duration()) {
    ...
    Eigen::Vector2d wheelVelocities =
        inverseKinematicsMatrix *
        Eigen::Vector3d{chassisVelocities.vx.value(),
...
TEST_CASE("TwoDeadWheelPoseEstimator3dTest AccuracyFacingTrajectory",
          "[wpimath]") {
...
TEST_CASE("TwoDeadWheelPoseEstimator3dTest BadInitialPose", "[wpimath]") {
  wpi::math::TwoDeadWheelPoseEstimator3d estimator{1_m,
                                                   1_m,
                                                   0_m,
                                                   0_m,
...
TEST_CASE("TwoDeadWheelPoseEstimator3dTest SimultaneousVisionMeasurements",
          "[wpimath]") {
  ...
  estimator.UpdateWithTime(0_s, 0_m, 0_m, wpi::math::Rotation3d{});
...
TEST_CASE("TwoDeadWheelPoseEstimator3dTest TestDiscardStaleVisionMeasurements",
          "[wpimath]") {
  wpi::math::TwoDeadWheelPoseEstimator3d estimator{1_m,
                                                   1_m,
                                                   0_m,
  ...
  for (auto time = 0_s; time < 4_s; time += 20_ms) {
    estimator.UpdateWithTime(time, 0_m, 0_m, wpi::math::Rotation3d{});
  }
  ...
  estimator.AddVisionMeasurement(
      wpi::math::Pose3d{wpi::math::Translation3d{10_m, 10_m, 0_m},
  ...
  CHECK_NEAR(odometryPose.Z().value(),
             estimator.GetEstimatedPosition().Z().value(), 1e-6);
  CHECK_NEAR(odometryPose.Rotation().X().value(),
             estimator.GetEstimatedPosition().Rotation().X().value(), 1e-6);
  CHECK_NEAR(odometryPose.Rotation().Y().value(),
             estimator.GetEstimatedPosition().Rotation().Y().value(), 1e-6);
  CHECK_NEAR(odometryPose.Rotation().Z().value(),
             estimator.GetEstimatedPosition().Rotation().Z().value(), 1e-6);
}

TEST_CASE("TwoDeadWheelPoseEstimator3dTest TestSampleAt", "[wpimath]") {
  wpi::math::TwoDeadWheelPoseEstimator3d estimator{1_m,
                                                   1_m,
                                                   0_m,
                                                   0_m,
...
TEST_CASE("TwoDeadWheelPoseEstimator3dTest TestReset", "[wpimath]") {
  wpi::math::TwoDeadWheelPoseEstimator3d estimator{
      1_m,
      1_m,
      0_m,
  ...
  CHECK_DOUBLE_EQ(-1, estimator.GetEstimatedPosition().Z().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().X().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().Y().value());
  CHECK_DOUBLE_EQ(1, estimator.GetEstimatedPosition().Rotation().Z().value());
  ...
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Z().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().X().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().Y().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().Z().value());

  // Test orientation and wheel positions
  estimator.Update(2_m, 0_m, wpi::math::Rotation3d{});
  ...
  // Test reset rotation
  estimator.ResetRotation(wpi::math::Rotation3d{90_deg});
  ...
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().X().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().Y().value());
  CHECK_DOUBLE_EQ(std::numbers::pi / 2,
                  estimator.GetEstimatedPosition().Rotation().Z().value());

  // Test orientation
  estimator.Update(3_m, 0_m, wpi::math::Rotation3d{});
  ...
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().X().value());
  CHECK_DOUBLE_EQ(0, estimator.GetEstimatedPosition().Rotation().Y().value());
  CHECK_DOUBLE_EQ(std::numbers::pi * 3.0 / 4,
                  estimator.GetEstimatedPosition().Rotation().Z().value());
