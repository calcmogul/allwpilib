...
Trajectory TrajectoryParameterizer::TimeParameterizeTrajectory(
  ...
  for (size_t k = 0; k < points.size(); ++k) {
    ...
    units::meters_per_second_squared_t linearAcceleration{a[k].value()};
    auto curvature = points[k].second;
    auto ω_k = linearVelocity * curvature;

    if (k < points.size() - 1) {
      states.emplace_back(t, points[k].first,
                          reversed ? -linearVelocity : linearVelocity,
                          reversed ? -linearAcceleration : linearAcceleration,
                          reversed ? -ω_k : ω_k);
      t += units::second_t{dt[k].value()};
    } else {
      states.emplace_back(t, points[k].first,
                          reversed ? -linearVelocity : linearVelocity, 0_mps_sq,
                          reversed ? -ω_k : ω_k);
    }
  }
