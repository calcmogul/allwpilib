...
Translation2d Ellipse2d::Nearest(const Translation2d& point) const {
  ...
  // Find nearest point
  {
    ...
    slp::SQPMatrixCallbacks<double> matrix_callbacks{
        2,
        1,
        ...
        [&](const Eigen::VectorXd& x,
          ...
          // ∇ₓₓ²L(x, y) = [2 − 2b²y     0    ]
          //               [   0      2 − 2a²y]
          return Eigen::SparseMatrix<double>{Eigen::DiagonalMatrix<double, 2>{
              2.0 - 2.0 * b2 * y[0], 2.0 - 2.0 * a2 * y[0]}};
        },
        [&](const Eigen::VectorXd& x,
            const Eigen::VectorXd& y) -> Eigen::SparseMatrix<double> {
          // L(x, y) = f(x) − yᵀcₑ(x)
