...
class ReturnByValue : public internal::dense_xpr_base<ReturnByValue<Derived> >::type, internal::no_assignment_operator {
 public:
  ...
  EIGEN_DEVICE_FUNC inline constexpr Index rows() const EIGEN_NOEXCEPT {
    return static_cast<const Derived*>(this)->rows();
  }
  EIGEN_DEVICE_FUNC inline constexpr Index cols() const EIGEN_NOEXCEPT {
...
#define Unusable \
  ...
  class Unusable {
    constexpr Unusable(const Unusable&) {}
    constexpr Unusable& operator=(const Unusable&) { return *this; }
  };
