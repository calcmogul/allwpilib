...
class WPILIB_DLLEXPORT DifferentialDriveVoltageConstraint
 ...
 public:
  ...
  constexpr DifferentialDriveVoltageConstraint(
    ...
    m_A = Eigen::Matrix<double, 2, 2>{{-Kv / Ka, 0.0}, {0.0, -Kv / Ka}};
  ...
  void Apply(slp::Problem& problem, const Pose2d& pose,
    ...
    auto rightWheelVelocity =
        linearVelocity + angularVelocity * m_trackwidth.value() / 2.0;
    slp::VariableMatrix x{{leftWheelVelocity}, {rightWheelVelocity}};
    ...
    auto rightWheelAcceleration =
        linearAcceleration + angularAcceleration * m_trackwidth.value() / 2.0;
    slp::VariableMatrix dxdt{{leftWheelAcceleration}, {rightWheelAcceleration}};

    slp::VariableMatrix c{{m_c(0) * slp::sign(x[0])},
                          {m_c(1) * slp::sign(x[1])}};

    auto u = m_Binv * (dxdt - m_A * x - c);

    problem.subject_to(u[0] >= -m_maxVoltage.value());
    problem.subject_to(u[0] < m_maxVoltage.value());
    problem.subject_to(u[1] >= -m_maxVoltage.value());
    problem.subject_to(u[1] < m_maxVoltage.value());
  }
