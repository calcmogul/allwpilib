...
class VariableBlock {
 public:
  ...
  VariableBlock<Mat>& operator=(const frc::Matrixd<_Rows, _Cols>& values) {
  ...
  template <int _Rows, int _Cols>
  VariableBlock<Mat>& operator=(frc::Matrixd<_Rows, _Cols>&& values) {
    for (size_t row = 0; row < _Rows; ++row) {
      for (size_t col = 0; col < _Cols; ++col) {
        Autodiff(row, col) = values(row, col);
      }
...
#include "frc/optimization/VariableBlock.inc"
