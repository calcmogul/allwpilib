...
template <class Distance>
  requires wpi::units::traits::is_length_unit_v<Distance> ||
           wpi::units::traits::is_angle_unit_v<Distance>
class DCMotorInputErrorEstimator {
 public:
  ...
  using Ka_t = wpi::units::unit_t<
      wpi::units::compound_unit<units::volt, wpi::units::inverse<Acceleration>>>;
   ...
   */
  wpi::units::volt_t Calculate(units::volt_t oldInput, Distance_t currentPosition) {
    m_kf.Predict(Vectord<1>{oldInput.value()}, m_dt);
    m_kf.Correct(Vectord<1>{oldInput.value()},
                 Vectord<1>{currentPosition.value()});

    return wpi::units::volt_t{m_kf.Xhat(2)};
  }
