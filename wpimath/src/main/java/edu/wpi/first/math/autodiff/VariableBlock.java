...
public class VariableBlock {
  private VariableMatrix m_mat;

  private Slice m_rowSlice = new Slice();
  private int m_rowSliceLength;

  private Slice m_colSlice = new Slice();
  private int m_colSliceLength;
  ...
  public VariableBlock slice(Slice rowSlice, Slice colSlice) {
    ...
    return new VariableBlock(
        ...
        new Slice(
            ...
            m_rowSlice.start + rowSlice.stop,
            m_rowSlice.step * rowSlice.step),
        rowSliceLength,
        new Slice(
            m_colSlice.start + colSlice.start * m_colSlice.step,
            m_colSlice.start + colSlice.stop,
            m_colSlice.step * colSlice.step),
        colSliceLength);
  }
  ...
  public double value(int row, int col) {
    assert row >= 0 && row < rows();
    assert col >= 0 && col < cols();
    return get(m_rowSlice.start + row * m_rowSlice.step, m_colSlice.start + col * m_colSlice.step)
        .value();
  }
