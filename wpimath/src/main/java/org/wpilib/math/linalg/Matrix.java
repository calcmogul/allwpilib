...
public class Matrix<R extends Num, C extends Num>
  ...
  private static boolean squareRoot(SimpleMatrix A, SimpleMatrix B) {
    // [1] Å. Björck and S. Hammarling, "A Schur method for the square root of a
    //     matrix," Linear Algebra and its Applications, vol. 52–53,
    //     pp. 127–140, Jul. 1983.
    ...
    // b_i,j = (t_i,j - b_i,i+1 b_i+1,j - b_i,i+2 b_i_2,j - … - b_i,j-1 b_j-1,j)/(b_i,i + b_j,j)
    //
  ...
  /**
   * Returns Padé approximant of (I - T)ᵖ.
   *
   * @param degree Padé approximant degree.
   * @param IminusT I - T where T is an upper triangular matrix.
   * @param exponent Exponent of (I - T)ᵖ.
   */
  private final SimpleMatrix computePade(int degree, SimpleMatrix IminusT, double exponent) {
    // See Algorithm 4.1 of [1].
    //
    // [1] N. J. Higham and L. Lin, "A Schur–Padé Algorithm for Fractional
    //     Powers of a Matrix," SIAM Journal on Matrix Analysis and
    //     Applications, vol. 32, no. 3, pp. 1056–1078, Jul. 2011.
    //     DOI: 10.1137/10081232X

    final double p = exponent;
    final var I = new SimpleMatrix(IminusT.getNumRows(), IminusT.getNumCols());

    int j = 2 * degree;
    var result = IminusT.scale((p - degree) / (2 * j - 2));

    --j;
    while (j > 0) {
      if (j == 1) {
        result = I.plus(result).solve(IminusT.scale(-p));
      } else if ((j & 1) != 0) {
        result = I.plus(result).solve(IminusT.scale((-p - (j / 2)) / (2 * j)));
      } else {
        result = I.plus(result).solve(IminusT.scale((p - (j / 2)) / (2 * j - 2)));
      }
      --j;
    }

    return result.plus(I);
  }

  private final int getPadeDegree(double normIminusT) {
    // Max norms for Padé approximants with degrees 3 through 7
    final var maxNormForPade =
        new double[] {
