...
public final class DARE {
  ...
  public static <States extends Num, Inputs extends Num> Matrix<States, States> dareNoPrecond(
    ...
    var R_llt = LinearSolverFactory_DDRM.chol(R.getNumRows());
  ...
  public static <States extends Num, Inputs extends Num> Matrix<States, States> dare(
    ...
    var Q_ldlt = DecompositionFactory_DDRM.cholLDL();
    if (!Q_ldlt.decompose(Q.getStorage().getDDRM())) {
      throw new IllegalArgumentException("Q was not positive semidefinite.\n\n" + Q);
    }
    var diag = Q_ldlt.getDiagonal();
    for (double elem : diag) {
      if (elem < 0.0) {
        throw new IllegalArgumentException("Q was not positive semidefinite.\n\n" + Q);
      }
    }
    ...
    // Q = CᵀC = LDLᵀ
    // C = √(D)Lᵀ
    var D = new SimpleMatrix(Q.getNumRows(), Q.getNumCols());
    Q_ldlt.getD(D.getDDRM());
    for (int row = 0; row < D.getNumRows(); ++row) {
      D.set(row, row, Math.sqrt(D.get(row, row)));
    }
    var L = new SimpleMatrix(Q.getNumRows(), Q.getNumCols());
    Q_ldlt.getL(L.getDDRM());
    var C = D.mult(L.transpose());
  ...
  public static <States extends Num, Inputs extends Num> Matrix<States, States> dare(
    ...
    if (!LinearSystemUtil.isDetectable(new Matrix<>(A_2), new Matrix<>(C))) {
      throw new IllegalArgumentException(
          "(A, C) pair where Q = CᵀC was not detectable.\n\n" + A_2 + "\n" + C);
    }
